...
We evaluated permutation-based Shapley values against Leave-One-Out (LOO) utility degradation across clients (Hospital 1, 2, and 3). Note that the Shapley values in Experiment 5 are computed for the baseline valuation-alignment analysis under a moderate label skew ($\alpha=1.0$), whereas Experiment 6 recomputes Shapley values independently under each privacy configuration:
\begin{itemize}
...
Without DP, the confidence-threshold MIA attacker achieves an AUC of 0.5679 (attacker advantage = 0.0734) on prediction confidence. With DP enabled, the attacker AUC decreases to 0.4802 at $\epsilon=10.0$ and declines further as the privacy budget is tightened (as mapped in Fig. \ref{fig:mia}). Thus, the evaluated confidence-threshold attack loses discriminative advantage under the applied privacy perturbation, with attacker performance approaching and falling below the random-guess baseline of 0.50, supporting \textbf{H7}.
\begin{figure}[ht]
\centering
