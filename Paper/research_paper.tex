...
\end{equation}
Because a single patient barcode can match multiple sample records in TCGA-PRAD, this definition enforces sample-level rather than patient-level differential privacy. Note that for our final 347-sample matched analysis cohort, sample-level and patient-level protection coincide with respect to this specific dataset because it has exactly one sample per patient. However, the privacy mechanism's adjacency and guarantees are formally defined relative to sample-level add/remove records, which remains the correct unit of protection relative to the broader data extraction mechanism.
...
H3 & Interaction of privacy \& heterogeneity & Test AUC Benefit ($\Delta$) & Small paired benefit ($+0.0020$ AUC) at $\epsilon=1.0/2.0$ in cell-level comparisons; no significant Method or interaction effect in 20-seed MixedLM (all $P \ge 0.795$). & Descriptive, not inferentially powered ($N=3$ clients) \\
