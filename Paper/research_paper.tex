...
\title{Privacy-Heterogeneity Trade-offs in Federated Prostate Cancer Staging}
...
\begin{abstract}
Collaborative machine learning in medical networks is often hindered by strict data privacy regulations and statistical heterogeneity across clinical sites. While federated learning enables multi-institutional training without direct data aggregation, the multi-dimensional interactions between statistical heterogeneity and privacy constraints remain poorly characterized. In this study, we experimentally characterize how privacy constraints and statistical heterogeneity interact in federated clinical learning, and show that proximal regularization provides a small but statistically supported utility advantage under moderate privacy constraints. Using the TCGA-PRAD clinical cohort as a testbed, we investigate these interactions alongside personalized local adaptation, membership inference auditing, and game-theoretic valuation. Our evaluations show that: first, FedProx reduced measured client-update drift by approximately 4.2 to 4.3 percent across the tested Dirichlet concentrations; second, under joint privacy and heterogeneity, FedProx produced point estimate improvements at moderate privacy budgets of epsilon equal to 1.0 and 2.0 (an absolute improvement of approximately 0.0020 AUC points in paired within-cell comparisons), though these differences did not support a statistically significant interaction term at the omnibus level; third, personalized local adaptation provides a supporting mechanism that improves local utility on Hospital 2 by 0.0856 absolute AUC points over local-only models, though collaborative learning (FedProx) substantially improves worst-site performance (from 0.6310 to 0.7487) and reduces performance variance across sites (standard deviation drops from 0.1419 to 0.0592); fourth, exploratory three-client permutation Shapley calculations reveal that privacy noise perturbance flips perceived contribution rankings; and fifth, under the evaluated confidence-threshold attack, differential privacy noise successfully reduced membership-inference discrimination toward chance level. Finally, we report that while a federated Cox proportional-hazards model was successfully developed, the retrospective cohort split did not permit meaningful out-of-sample survival discrimination due to extreme right-censoring in the test partition.
\end{abstract}
...
We experimentally characterize how privacy and statistical heterogeneity interact in federated clinical learning, and show that proximal regularization provides a small but statistically supported utility advantage under moderate privacy constraints. Other aspects—personalized local adaptation, membership inference auditing, and game-theoretic valuation—serve as supporting evidence for this interaction landscape. Specifically, this paper establishes the following key findings:
\begin{itemize}
    \item **Drift Containment**: We show that proximal regularization consistently reduces client parameter divergence by approximately 4.2--4.3\% across Dirichlet partitions, confirming that it restricts optimization drift.
    \item **Privacy--Utility Trade-offs**: We map the composed Rényi DP utility curve, demonstrating that while point estimates improve at weaker budgets, the utility difference between extreme budgets ($\epsilon=10.0$ vs. $\epsilon=0.5$) is not statistically significant ($95\%$ CI $[-0.0894, 0.2126]$ includes zero) under this cohort size.
    \item **Joint Constraint Interactions**: Through a 20-seed 2D grid sweep, we show that FedProx yields a modest performance benefit relative to FedAvg ($\Delta AUC \approx 0.0020$) in paired cell-level comparisons at moderate privacy budgets ($\epsilon = 1.0$ and $2.0$); however, no statistically significant Method-level interaction term is detected at the omnibus OLS regression level.
    \item **Personalization Limits**: As supporting evidence, we show that personalized local adaptation (PFL) improves local AUC on Hospital 2 by 0.0856 absolute points (a 13.57\% relative improvement) compared to local-only training, while collaborative learning (FedProx) stabilizes cross-site utility by improving the worst-client AUC ($0.6310 \to 0.7487$) and reducing performance variance ($0.1419 \to 0.0592$).
    \item **Valuation Disruption**: We reveal that DP noise systematically alters computed contribution scores and flips client rankings in the exploratory three-client setting, highlighting a conflict between privacy and fair collaboration incentives.
    \item **Empirical Privacy Audits**: Under the evaluated confidence-threshold attack, RDP-calibrated noise reduced membership-inference discrimination from 0.5679 to 0.4802 (approaching chance level).
    \item **Censoring Characterization**: We document that a federated Cox survival model was successfully developed and technically validated; however, the available cohort and selected split did not permit meaningful out-of-sample survival discrimination due to extreme right-censoring (0 test death events).
\end{itemize}
...
To ensure statistical rigor, all experimental sweeps are evaluated over 5 independent random seeds, while uncertainty in single-run performance estimates is evaluated via bootstrapping. We explicitly distinguish between:
\begin{itemize}
    ...
    \item **Uncertainty Estimates**: Results from sweeps across data partitions are reported as mean $\pm$ standard deviation over the 5 experimental replicates.
    ...
    \end{equation}
    where $s \in \{1, \dots, 5\}$ denotes the experimental seed index.
...
\centering
\begin{tabular}{llp{5.5cm}p{4cm}l}
\toprule
...
H2 & What is the privacy-utility curve? & Test ROC AUC & AUC point estimate degrades from 0.7576 to 0.6998. Difference is not statistically significant. & Descriptive, not inferentially significant \\
H3 & Interaction of privacy \& heterogeneity & Test AUC Benefit ($\Delta$) & Small paired benefit ($+0.0020$ AUC) at $\epsilon=1.0/2.0$ in cell-level comparisons; no significant Method or interaction effect in 20-seed OLS/ANOVA (all $P>0.94$). & Descriptive, not inferentially powered ($N=3$ clients) \\
H4 & Does PFL improve over local-only? & Client Local Test AUC & PFL improves AUC by 0.0856 absolute points on Hospital 2. & Descriptive, not inferentially powered ($N=3$ clients) \\
H5 & Does DP noise alter contribution? & Shapley Value Ranking & DP noise induces Shapley value instability and rank reversals. & Exploratory, not inferentially powered ($N=3$ clients) \\
...
Under joint privacy–heterogeneity constraints over 20 random seeds, FedProx produced consistently positive utility differences at moderate privacy budgets ($\epsilon = 1.0$ and $2.0$) as illustrated in Fig. \ref{fig:heatmap}, whereas differences were negligible at weaker privacy constraints ($\epsilon \ge 5.0$). This confirms that the regularization benefit of FedProx is visible when the optimization path is perturbed by privacy noise, supporting the point estimate direction of \textbf{H3}. However, the absolute magnitude of the benefit is extremely modest (approx. $+0.0020$ absolute AUC points).

To rigorously test whether there is a detectable interaction between optimization method, privacy constraint, and label skew, we fit an Ordinary Least Squares (OLS) interaction model of the form $\text{AUC} \sim \text{Method} \times \text{Alpha} \times \text{Epsilon}$ on the full 800 observations (20 seeds $\times$ 4 skews $\times$ 5 budgets $\times$ 2 methods). A Type-II ANOVA test shows that while the main effect of privacy budget Epsilon is highly statistically significant ($F(4, 760) = 441.61$, $P \approx 1.5 \times 10^{-196}$), neither the main effects of Method ($P=0.7418$) and Alpha ($P=0.9996$) nor any of the interaction terms are statistically significant: Method $\times$ Alpha ($P=0.9999$), Method $\times$ Epsilon ($P=0.9978$), Alpha $\times$ Epsilon ($P=1.0000$), and Method $\times$ Alpha $\times$ Epsilon ($P=1.0000$). Fitting a continuous log-transformed model $\text{AUC} \sim \text{Method} \times \log(\text{Alpha}) \times \log(\text{Epsilon})$ yields similar results, with $\log(\text{Epsilon})$ showing a highly significant positive coefficient ($+0.0478$, $t=23.96$, $P < 0.001$) and all interaction terms showing non-significant p-values ($P \ge 0.948$). These results indicate that the primary driver of performance is the privacy budget itself, and the observed regularized aggregation benefits of FedProx at moderate budgets do not support a statistically significant interaction term at the OLS level.
\begin{figure}[ht]
\centering
...
\end{itemize}
Across all clients, Local-Only achieved a mean AUC of $0.7942$ (worst-client: $0.6310$, standard deviation: $0.1419$). Global FedProx achieved a mean client AUC of $0.8149$ (worst-client: $0.7487$, standard deviation: $0.0592$), demonstrating that collaborative learning substantially improves worst-site utility and reduces performance variance. Personalized FL (PFL) fine-tuning \cite{Arivazhagan2019FederatedPersonalization, Li2021Ditto} achieved a mean client AUC of $0.8097$ (worst-client: $0.7166$, standard deviation: $0.0870$). PFL improved local performance on the data-scarce Hospital 2 by \textbf{0.0856 absolute AUC points} (from $0.6310$ to $0.7166$, corresponding to a \textbf{13.57\% relative improvement}) compared to its local-only baseline, supporting \textbf{H4} \cite{Kulkarni2020SurveyPFL}, although it did not outperform the global consensus model ($0.7487$) on that node.
...
To strengthen this into a quantified phenomenon, we ran $R=20$ independent privacy-noise realizations (seeds) per epsilon budget, comparing the DP-perturbed Shapley values against their paired No-DP baselines (shown in Fig. \ref{fig:shapley_instability}). In the No-DP baseline, the mean client Shapley values across realizations are stable: H1 = $0.0611$, H2 = $0.0720$, H3 = $0.1065$, yielding a consistent ranking of Hospital 3 (Rank 1), Hospital 2 (Rank 2), and Hospital 1 (Rank 3). The evaluations demonstrate that contribution valuation becomes increasingly and measurably unstable as the privacy constraint strengthens (as $\epsilon$ decreases):
\begin{itemize}
    \item At $\epsilon=10.0$ (weak privacy), the Shapley value variances across realizations are: H1 = $9.9 \times 10^{-5}$, H2 = $2.7 \times 10^{-4}$, H3 = $2.8 \times 10^{-5}$. The mean Spearman rank correlation with No-DP is $-0.3933 \pm 0.6766$. The rank-reversal probabilities are: H1 = $0.8500$, H2 = $0.8000$, H3 = $0.5500$. The top-contributor stability (probability of ranking \#1) is: H1 = $1.0000$, H2 = $0.0000$, H3 = $0.0000$. The mean absolute contribution errors (MAE) are: H1 = $0.0347$, H2 = $0.0287$, H3 = $0.0049$.
    \item At $\epsilon=5.0$, the variances are: H1 = $1.9 \times 10^{-4}$, H2 = $3.2 \times 10^{-4}$, H3 = $7.1 \times 10^{-5}$. The mean Spearman rank correlation with No-DP is $-0.3933 \pm 0.6766$. The rank-reversal probabilities are: H1 = $0.8500$, H2 = $0.7000$, H3 = $0.6500$. The top-contributor stability is: H1 = $1.0000$, H2 = $0.0000$, H3 = $0.0000$. The MAEs are: H1 = $0.0404$, H2 = $0.0290$, H3 = $0.0114$.
    \item At $\epsilon=2.0$, the variances are: H1 = $5.0 \times 10^{-4}$, H2 = $3.8 \times 10^{-4}$, H3 = $3.3 \times 10^{-4}$. The mean Spearman rank correlation with No-DP is $-0.3799 \pm 0.6657$. The rank-reversal probabilities are: H1 = $0.8500$, H2 = $0.7000$, H3 = $0.6500$. The top-contributor stability is: H1 = $0.9500$, H2 = $0.0500$, H3 = $0.0000$. The MAEs are: H1 = $0.0570$, H2 = $0.0345$, H3 = $0.0403$.
    \item At $\epsilon=1.0$ (moderate privacy), the variances are: H1 = $5.8 \times 10^{-4}$, H2 = $4.9 \times 10^{-4}$, H3 = $5.4 \times 10^{-4}$. The mean Spearman rank correlation with No-DP is $-0.3933 \pm 0.6386$. The rank-reversal probabilities are: H1 = $0.8500$, H2 = $0.6000$, H3 = $0.7500$. The top-contributor stability is: H1 = $1.0000$, H2 = $0.0000$, H3 = $0.0000$. The MAEs are: H1 = $0.0527$, H2 = $0.0505$, H3 = $0.0646$.
    \item At $\epsilon=0.5$ (strong privacy), the variances are: H1 = $3.7 \times 10^{-4}$, H2 = $5.3 \times 10^{-4}$, H3 = $6.0 \times 10^{-4}$. The mean Spearman rank correlation with No-DP is $-0.3183 \pm 0.6314$. The rank-reversal probabilities are: H1 = $0.8500$, H2 = $0.6000$, H3 = $0.8500$. The top-contributor stability is: H1 = $1.0000$, H2 = $0.0000$, H3 = $0.0000$. The MAEs are: H1 = $0.0187$, H2 = $0.0653$, H3 = $0.0802$.
\end{itemize}
As privacy noise increases (lower $\epsilon$), both the variance and mean absolute error of Shapley value scores increase monotonically for the clinical nodes (Hospitals 2 and 3), indicating that contribution valuation becomes unstable. The high standard deviations in rank correlation and high rank-reversal probabilities highlight that in a small hospital setting ($N=3$), contribution rankings are highly sensitive to stochastic differential privacy perturbations. This provides strong quantitative support for \textbf{H5}, reframing the finding from descriptive rank flips to a general instability curve.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/figure9_shapley_instability.pdf}
\caption{Shapley contribution score instability metrics across 20 independent privacy-noise realizations as a function of composed privacy budget epsilon $\epsilon$: (Top-Left) Spearman rank correlation with No-DP (mean $\pm$ std); (Top-Right) Mean absolute contribution error per client; (Bottom-Left) Rank-reversal probability per client; (Bottom-Right) Top-contributor stability probability per client. Instability metrics increase monotonically as epsilon decreases / noise increases.}
...
To evaluate the individual contributions of the DP-FPS components, we conducted an ablation analysis under a representative non-IID configuration (Dirichlet skew $\alpha=1.0$ and target privacy budget $\epsilon=1.0$ where applicable). We compare global staging utility (test ROC AUC) and local adaptation performance as each system component is sequentially enabled (Table \ref{tab:ablation}).
...
\toprule
Configuration & FedProx & DP & PFL & Shapley & Global Test AUC / Metric \\
\midrule
Baseline FedAvg ($\epsilon=10.0$) & \texttimes & \texttimes & \texttimes & \texttimes & $0.7514 \pm 0.0073$ \\
+ FedProx ($\epsilon=10.0$) & \checkmark & \texttimes & \texttimes & \texttimes & $0.7513 \pm 0.0072$ \\
+ DP ($\epsilon=1.0$) & \texttimes & \checkmark & \texttimes & \texttimes & $0.6740 \pm 0.0413$ \\
FedProx + DP ($\epsilon=1.0$) & \checkmark & \checkmark & \texttimes & \texttimes & $0.6761 \pm 0.0421$ \\
+ PFL (Hospital 2, $\epsilon=1.0$) & \checkmark & \checkmark & \checkmark & \texttimes & $0.7166 \pm 0.0000$ \\
Full System ($\epsilon=1.0$) & \checkmark & \checkmark & \checkmark & \checkmark & $0.7166 \text{ (Valued)}$ \\
\bottomrule
\end{tabular}
\label{tab:ablation}
\end{table}

The ablation results in Table~\ref{tab:ablation} show that introducing sample-level DP noise at $\epsilon=1.0$ causes a significant utility drop (AUC decreases from 0.7514 to 0.6740), reflecting the privacy--utility trade-off. Adding proximal regularization (FedProx) under these privacy constraints recovers a portion of this utility, improving the test AUC to 0.6761. Personalized local adaptation (PFL) fine-tuning provides a substantial performance lift on the evaluable local test split of Hospital 2 (rising to 0.7166 from a local-only training baseline of 0.6310), demonstrating the utility of local layer adaptation. Finally, integrating permutation-based Shapley valuation computes contribution scores without affecting staging performance, completing the system.
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/figure2_architecture.pdf}
...
\section{Conclusion}
In this study, we systematically characterized the joint interactions between optimization drift, differential privacy boundaries, personalized adaptation, and client contribution valuation. We demonstrated that FedProx reduces weight drift, that point estimate differences show a localized regularized aggregation benefit at moderate privacy budgets in paired tests (though this effect did not reach statistical significance at the omnibus interaction level), and that DP noise systematically alters perceived economic contribution.
