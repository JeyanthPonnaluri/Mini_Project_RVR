...
\begin{itemize}
    ...
    \item **Privacy--Utility Trade-offs**: We map the composed Rényi DP utility curve, demonstrating that moderate budgets ($\epsilon \in [1.0, 2.0]$) preserve staging performance, although the absolute staging performance has wide confidence intervals due to cohort data scarcity.
    \item **Joint Constraint Interactions**: Through a 5-seed 2D grid sweep, we show that FedProx yields a consistent but modest performance benefit relative to FedAvg ($\Delta AUC \approx 0.0028 - 0.0045$) at moderate privacy budgets ($\epsilon = 1.0$ and $2.0$). We explicitly acknowledge that this magnitude is modest and does not represent clinically meaningful improvement.
    \item **Personalization Limits**: As supporting evidence, we show that personalized local adaptation (PFL) improves AUC by 0.0786 absolute points (a 12.45\% relative improvement) compared to local-only training, but fails to outperform the global consensus model in data-scarce settings.
...
H3 & Interaction of privacy \& heterogeneity & Test AUC Benefit ($\Delta$) & FedProx improves AUC at $\epsilon=1.0$ and $2.0$; CI is positive. & Descriptive, not inferentially powered ($N=3$ clients) \\
...
Under joint privacy–heterogeneity constraints over repeated trials, FedProx produced consistently positive utility differences at moderate privacy budgets ($\epsilon = 1.0$ and $2.0$) as illustrated in Fig. \ref{fig:heatmap}, whereas differences were small and sometimes negative at weaker privacy constraints ($\epsilon = 5.0$ and $10.0$). This confirms that the regularization benefit of FedProx is most visible when the optimization path is perturbed by privacy noise under label heterogeneity, supporting **H3**. Although the observed FedProx advantage is statistically consistent under the evaluated conditions, its absolute magnitude is modest (ranging from 0.0028 to 0.0045 AUC points) and should not be interpreted as evidence of clinically meaningful improvement.
\begin{figure}[ht]
\centering
...
To strengthen this into a quantified phenomenon, we ran $R=20$ independent privacy-noise realizations (seeds) per epsilon budget, comparing the DP-perturbed Shapley values against their paired No-DP baselines (shown in Fig. \ref{fig:shapley_instability}). The evaluations demonstrate that contribution valuation becomes increasingly and measurably unstable as the privacy constraint strengthens (as $\epsilon$ decreases):
\begin{itemize}
    ...
    \item At $\epsilon=1.0$ (moderate privacy), the variances are: H1 = $5.8 \times 10^{-4}$, H2 = $4.9 \times 10^{-4}$, H3 = $5.4 \times 10^{-4}$. The rank-reversal probabilities are: H1 = $0.8500$, H2 = $0.6000$, H3 = $0.7500$. The MAEs increase to: H1 = $0.0527$, H2 = $0.0505$, H3 = $0.0646$.
    \item At $\epsilon=0.5$ (strong privacy), the variances are: H1 = $3.7 \times 10^{-4}$, H2 = $5.3 \times 10^{-4}$, H3 = $6.0 \times 10^{-4}$. The rank-reversal probabilities are: H1 = $0.8500$, H2 = $0.6000$, H3 = $0.8500$. The MAEs are: H1 = $0.0187$, H2 = $0.0653$, H3 = $0.0802$.
\end{itemize}
...
\midrule
Baseline FedAvg & \texttimes & \texttimes & \texttimes & \texttimes & $0.7561 \pm 0.0046$ \\
+ FedProx & \checkmark & \texttimes & \texttimes & \texttimes & $0.7547 \pm 0.0056$ \\
+ DP & \texttimes & \checkmark & \texttimes & \texttimes & $0.6892 \pm 0.0487$ \\
FedProx + DP & \checkmark & \checkmark & \texttimes & \texttimes & $0.6936 \pm 0.0485$ \\
+ PFL (Hospital 2) & \checkmark & \checkmark & \checkmark & \texttimes & $0.7101 \pm 0.0000$ \\
Full System & \checkmark & \checkmark & \checkmark & \checkmark & $0.7101 \text{ (Valued)}$ \\
\bottomrule
\end{tabular}
\label{tab:ablation}
\end{table}

The ablation results in Table~\ref{tab:ablation} show that introducing sample-level DP noise at $\epsilon=1.0$ causes a significant utility drop (AUC decreases from 0.7561 to 0.6892), reflecting the privacy--utility trade-off. Adding proximal regularization (FedProx) under these privacy constraints recovers a portion of this utility, improving the test AUC to 0.6936. Personalized local adaptation (PFL) fine-tuning provides a substantial performance lift on the evaluable local test split of Hospital 2 (rising to 0.7101 from a local-only training baseline of 0.6315), demonstrating the utility of local layer adaptation. Finally, integrating permutation-based Shapley valuation computes contribution scores without affecting staging performance, completing the system.
