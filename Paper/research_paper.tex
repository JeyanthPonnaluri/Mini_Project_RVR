...
To maintain the stability of clinical federated networks, contribution valuation methods are required to incentivize hospital participation \cite{Kazlouski2025HospitalParticipation, Wang2020ContributionSV}. Leave-One-Out (LOO) metrics assess a client's marginal value by comparing global performance with and without that client \cite{Ghorbani2019DataShapley}. However, LOO does not account for sub-coalition synergies and exhibits high variance. Game theory offers the Shapley value, which assigns contributions based on axiomatic fairness \cite{Jia2019KNNShapley}. However, computing exact Shapley values requires training models for all $2^K$ client coalitions, which is computationally expensive \cite{Lyu2020FairnessFL, Song2019BlockchainFL, Yan2021SelectionFL}.
...
\toprule
Study & Healthcare Tabular & Non-IID Skew & Differential Privacy & Personalization & Shapley Valuation & Privacy Attack Audit \\
\midrule
McMahan et al. \cite{McMahan2017FedAvg} & No & Yes & No & No & No & No \\
Li et al. \cite{Li2020FedProx} & No & Yes & No & No & No & No \\
Abadi et al. \cite{Abadi2016DPSGD} & No & No & Yes & No & No & No \\
Ghorbani et al. \cite{Ghorbani2019DataShapley} & No & No & No & No & Yes & No \\
Kazlouski et al. \cite{Kazlouski2025HospitalParticipation} & Yes & Yes & No & No & Yes & No \\
\textbf{Ours (DP-FPS)} & \textbf{Yes} & \textbf{Yes} & \textbf{Yes (RDP)} & \textbf{Yes} & \textbf{Yes} & \textbf{Yes (MIA)} \\
\bottomrule
\end{tabular}
...
To provide sample-level privacy guarantees, local gradients are clipped and perturbed using a Gaussian noise mechanism calibrated via Rényi Differential Privacy (RDP). Client contributions are evaluated using cooperative game theory. Let $S \subseteq \{1, \dots, K\}$ be a client coalition, and $v(S)$ be the utility of the model trained on coalition $S$, evaluated on a held-out validation set. The contribution of client $k$ is measured by its Shapley value $\phi_k$, reflecting its marginal utility across coalitions.
...
For a full local batch update, the sampling rate is $q=1.0$ (i.e., $B = n_k$). Under full batching, the step is not amplified by Poisson subsampling. For a Gaussian mechanism with noise multiplier $\sigma$, the step privacy loss at Rényi order $\alpha > 1$ is:
\begin{equation}
...
H7 & Does DP mitigate MIA? & MIA Attacker ROC AUC & Attacker AUC drops from 0.5679 to 0.4802. & Attack-specific \\
\bottomrule
\end{tabular}
